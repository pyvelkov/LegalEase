...
router.get("/", async (req, res) => {
    ...
    try {
        ...
        console.error(error);
        res.status(500).send("Error getting ALL templates metadata from DB: " + error);
    } finally {
        await dbClient.end();
    }
...
router.get("/:uuid", async (req, res) => {
    ...
    try {
        ...
        console.error(error);
        res.status(500).send("Error getting template metadata from DB: " + error);
    } finally {
        await dbClient.end();
    }
 ...
 * ================= */
 router.post("/:uuid", fileUpload.single("templateFile"), async (req, res) => {
    ...
    try{
        ...
        await storageClient.bucket("legalease").file(templatePath).save(Readable.from(templateFile));
    }
    catch(error){
        console.error(error);
    ...
    try {
        ...
        const templateFieldQueryRes = await dbClient.query(templateFieldSqlQuery);
